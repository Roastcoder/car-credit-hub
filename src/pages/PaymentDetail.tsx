...
export default function PaymentDetail() {
  ...
  const canProcess = user?.role === 'accounts' && payment.status === 'manager_approved';
  const canAddUTR = user?.role === 'accounts' && payment.status === 'accounts_processing';
    ...
    <>
      <div className="max-w-6xl mx-auto px-4 pb-20 lg:pb-4">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
          ...
          {/* Banking Details */}
          <Section title="Banking Details" icon={<Building2 size={20} />}>
            <div className="grid grid-cols-1 gap-4">
              <Field label="Beneficiary Name" value={payment.beneficiary_name} />
          ...
          {/* Loan Information */}
          <Section title="Loan Information" icon={<FileText size={20} />}>
            <div className="grid grid-cols-2 gap-4">
              <Field label="Loan Number" value={payment.loan_number} />
              <Field label="Customer Name" value={payment.applicant_name} />
              <Field label="Loan Amount" value={formatCurrency(Number(payment.loan_amount || 0))} />
              <Field label="Vehicle" value={`${payment.maker_name || ''} ${payment.model_variant_name || ''}`.trim() || '—'} />
            </div>
          </Section>
